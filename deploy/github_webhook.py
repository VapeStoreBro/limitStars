...
REPOSITORY = os.environ.get("GITHUB_REPOSITORY", "VapeStoreBro/limitStars")
WEBHOOK_SECRET = os.environ["GITHUB_WEBHOOK_SECRET"].encode()
PATH_SECRET = os.environ["DEPLOY_PATH_SECRET"]
PROJECT_DIR = Path(os.environ.get("PROJECT_DIR", "/root/limitstarsbot"))
SERVICE = os.environ.get("SYSTEMD_SERVICE", "limitstarsbot.service")
HOST = os.environ.get("DEPLOY_HOST", "0.0.0.0")
PORT = int(os.environ.get("DEPLOY_PORT", "9103"))
LOCK = asyncio.Lock()
...
async def deploy(request: web.Request) -> web.Response:
    ...
    async with LOCK:
        commands = [
            ...
            (str(PROJECT_DIR / ".venv/bin/python"), "-m", "compileall", "-q", "bot", "deploy", "run.py"),
...
async def health(_: web.Request) -> web.Response:
    return web.json_response({"ok": True, "service": "limitstars-deploy"})
